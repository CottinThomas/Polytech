...
int main(int argc, char * * argv)
{
	...
	// avg3 SSE
	{
		...
		{
			vfloat32_t vprev = _mm_load_ps(&v[i] - 4);
			vfloat32_t ve = _mm_load_ps(&v[i]);
			vfloat32_t vnext = _mm_load_ps(&v[i] + 4);
			
			vfloat32_t a = vfloat32_right_1(ve, vprev);
			vfloat32_t b = vfloat32_left_1(ve, vnext);

			vfloat32_t tmp = _mm_add_ps(ve, _mm_add_ps(a,b));
			tmp = _mm_div_ps(tmp, vfloat32_three);
